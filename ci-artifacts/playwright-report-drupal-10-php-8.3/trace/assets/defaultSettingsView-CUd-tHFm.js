...
const p0 = Object.defineProperty;
const m0 = (t, e, n) =>
  e in t
    ? p0(t, e, { enumerable: !0, configurable: !0, writable: !0, value: n })
    : (t[e] = n);
const Ee = (t, e, n) => m0(t, typeof e !== 'symbol' ? `${e}` : e, n);
(function () {
...
function y0() {
  ...
  const e = Symbol.for('react.portal');
  const n = Symbol.for('react.fragment');
  const r = Symbol.for('react.strict_mode');
  ...
  const d = Symbol.for('react.suspense');
  ...
  function K(I) {
    const U = { '=': '=0', ':': '=2' };
    return `$${I.replace(/[=:]/g, function (de) {
      return U[de];
    })}`;
  }
  const O = /\/+/g;
  ...
  return (
    ...
    (me.cloneElement = function (I, U, de) {
      ...
      if (_e === 1) fe.children = de;
        ...
        he = Array(_e);
        for (let ct = 0; ct < _e; ct++) he[ct] = arguments[ct + 2];
        fe.children = he;
      }
      return {
        $$typeof: t,
...
function Ar() {
  ...
  const [e, n] = Mt.useState(new DOMRect(0, 0, 10, 10));
  return (
    Mt.useLayoutEffect(() => {
      const r = t.current;
      if (!r) return;
...
const Np = '\\u0000-\\u0020\\u007f-\\u009f';
const Om = new RegExp(
  `(?:[a-zA-Z][a-zA-Z0-9+.-]{2,}:\\/\\/|www\\.)[^\\s${Np}"]{2,}[^\\s${
    Np
  }"')}\\],:;.!?]`,
  'ug',
);
...
function E0() {
  const t = Lu();
  const e = t === 'dark-mode' ? 'light-mode' : 'dark-mode';
    ...
    Ap ||
      ...
      (function (t) {
        ...
        function r(J) {
          ...
          if (Z !== se) {
            J[0] = Z;
            for (let I = 0, U = J.length, de = U >>> 1; I < de; ) {
              const fe = 2 * (I + 1) - 1;
              const pe = J[fe];
              const ye = fe + 1;
              const Se = J[ye];
        ...
        function o(J, se) {
          const Z = J.sortIndex - se.sortIndex;
        ...
        let k = !1;
        let _ = !1;
        const E = typeof setTimeout === 'function' ? setTimeout : null;
        const C = typeof clearTimeout === 'function' ? clearTimeout : null;
...
function C0() {
  ...
  function n(s) {
    ...
    return `Minified React error #${s}; visit ${
      i
  ...
  function B(s, i, a, f) {
    let h = E.hasOwnProperty(i) ? E[i] : null;
    (h !== null
      ? h.type !== 0
      : f ||
        !(i.length > 2) ||
  ...
  const R = t.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
  const D = Symbol.for('react.element');
  const z = Symbol.for('react.portal');
  const H = Symbol.for('react.fragment');
  const F = Symbol.for('react.strict_mode');
  const M = Symbol.for('react.profiler');
  const G = Symbol.for('react.provider');
  const K = Symbol.for('react.context');
  const O = Symbol.for('react.forward_ref');
  const X = Symbol.for('react.suspense');
  const ce = Symbol.for('react.suspense_list');
  const Ae = Symbol.for('react.memo');
  const be = Symbol.for('react.lazy');
  const ge = Symbol.for('react.offscreen');
...
${h[x].replace(' at new ', ' at ')}`;
  ...
  function ct(s) {
    const i = _e(s) ? 'checked' : 'value';
    const a = Object.getOwnPropertyDescriptor(s.constructor.prototype, i);
    let f = `${s[i]}`;
    if (
      !s.hasOwnProperty(i) &&
      typeof a < 'u' &&
  ...
  function hn(s, i) {
    let a = i.defaultValue == null ? '' : i.defaultValue;
    const f = i.checked != null ? i.checked : i.defaultChecked;
  ...
  function Sv(s, i, a, f, h, m, x, b, T) {
    const P = Array.prototype.slice.call(arguments, 3);
    try {
      i.apply(a, P);
    } catch (V) {
      this.onError(V);
  ...
  const Jf = e.unstable_scheduleCallback;
  const Xf = e.unstable_cancelCallback;
  const bv = e.unstable_shouldYield;
  const Tv = e.unstable_requestPaint;
  const De = e.unstable_now;
  const Cv = e.unstable_getCurrentPriorityLevel;
  const Na = e.unstable_ImmediatePriority;
  const Yf = e.unstable_UserBlockingPriority;
  const lo = e.unstable_NormalPriority;
  const Nv = e.unstable_LowPriority;
  const Zf = e.unstable_IdlePriority;
  let ao = null;
  let sn = null;
  function Av(s) {
  ...
  const Wt = Math.clz32 ? Math.clz32 : Mv;
  const Iv = Math.log;
  const Lv = Math.LN2;
  function Mv(s) {
  ...
  function fo(s, i) {
    let a = s.pendingLanes;
    if (a === 0) return 0;
    let f = 0;
    let h = s.suspendedLanes;
    let m = s.pingedLanes;
    let x = a & 268435455;
    if (x !== 0) {
      const b = x & ~h;
  ...
  function td(s) {
    return (
      (s &= -s),
      s > 1 ? (s > 4 ? ((s & 268435455) !== 0 ? 16 : 536870912) : 4) : 1
    );
  }
  let nd;
  let Ma;
  ...
  function po(s) {
    ...
    for (let i = s.targetContainers; i.length > 0; ) {
      let a = Oa(s.domEventName, s.eventSystemFlags, i[0], s.nativeEvent);
      if (a === null) {
        a = s.nativeEvent;
        const f = new a.constructor(a.type, a);
  ...
  function Ys(s) {
    ...
    for (; $n.length > 0 && ((a = $n[0]), a.blockedOn === null); )
      (ld(a), a.blockedOn === null && $n.shift());
  }
  const Br = R.ReactCurrentBatchConfig;
  let mo = !0;
  function Fv(s, i, a, f) {
    const h = xe;
    const m = Br.transition;
    Br.transition = null;
    try {
      ((xe = 1), Pa(s, i, a, f));
    } finally {
  ...
  function ud() {
    ...
    const a = i.length;
    let f;
    const h = 'value' in Rn ? Rn.value : Rn.textContent;
    const m = h.length;
  ...
  const dd = Ct(So);
  const Hv = { ...So, dataTransfer: 0 };
  const Uv = Ct(Hv);
  const qv = { ...Zs, relatedTarget: 0 };
  const Ba = Ct(qv);
  const Vv = { ...zr, animationName: 0, elapsedTime: 0, pseudoElement: 0 };
  const Wv = Ct(Vv);
  const Kv = {
    ...zr,
    clipboardData(s) {
  ...
  const sw = Ct(rw);
  const iw = { ...zr, propertyName: 0, elapsedTime: 0, pseudoElement: 0 };
  const ow = Ct(iw);
  const lw = {
    ...So,
    deltaX(s) {
  ...
  const aw = Ct(lw);
  const cw = [9, 13, 27, 32];
  const Ha = u && 'CompositionEvent' in window;
  let ti = null;
  u && 'documentMode' in document && (ti = document.documentMode);
  const uw = u && 'TextEvent' in window && !ti;
  const md = u && (!Ha || (ti && ti > 8 && ti <= 11));
  const gd = ' ';
  let yd = !1;
  function vd(s, i) {
    switch (s) {
  ...
  function Sd(s) {
    const i = s && s.nodeName && s.nodeName.toLowerCase();
  ...
  const Kt = typeof Object.is === 'function' ? Object.is : Sw;
  function si(s, i) {
    if (Kt(s, i)) return !0;
    if (
      typeof s !== 'object' ||
    ...
    if (
      ...
      if (f !== null && Va(a)) {
        if (
          ...
          s = s.getSelection();
          let h = a.textContent.length;
          let m = Math.min(f.start, h);
  ...
  const _w = u && 'documentMode' in document && document.documentMode <= 11;
  let Ur = null;
  let Wa = null;
  let ii = null;
  let Ka = !1;
  ...
  const Md = Eo('animationend');
  const jd = Eo('animationiteration');
  const Pd = Eo('animationstart');
  const Od = Eo('transitionend');
  const $d = new Map();
  const Rd =
  ...
  for (let Qa = 0; Qa < Rd.length; Qa++) {
    const Ja = Rd[Qa];
    const Ew = Ja.toLowerCase();
    const kw = Ja[0].toUpperCase() + Ja.slice(1);
    Dn(Ew, `on${kw}`);
  }
  ...
  const ko = `_reactListening${Math.random().toString(36).slice(2)}`;
  function li(s) {
    if (!s[ko]) {
      ((s[ko] = !0),
      ...
      const i = s.nodeType === 9 ? s : s.ownerDocument;
  ...
  function Ya(s, i, a, f, h) {
    ...
    if ((i & 1) === 0 && (i & 2) === 0 && f !== null)
      e: for (;;) {
        ...
        let x = f.tag;
        if (x === 3 || x === 4) {
          let b = f.stateNode.containerInfo;
    ...
    Vf(function () {
      ...
      e: {
        ...
        if (q !== void 0) {
          ...
          var ne = (i & 4) !== 0;
          var Fe = !ne && s === 'scroll';
          var L = ne ? (q !== null ? `${q}Capture` : null) : q;
          ne = [];
...
`,
  ...
  const nc = typeof setTimeout === 'function' ? setTimeout : void 0;
  const Nw = typeof clearTimeout === 'function' ? clearTimeout : void 0;
  const Ud = typeof Promise === 'function' ? Promise : void 0;
  const Aw =
  ...
  const Wr = Math.random().toString(36).slice(2);
  const on = `__reactFiber$${Wr}`;
  const ci = `__reactProps$${Wr}`;
  var pn = `__reactContainer$${Wr}`;
  var sc = `__reactEvents$${Wr}`;
  const Lw = `__reactListeners$${Wr}`;
  const Mw = `__reactHandles$${Wr}`;
  function or(s) {
    let i = s[on];
    if (i) return i;
    for (let a = s.parentNode; a; ) {
      if ((i = a[pn] || a[on])) {
        if (
          ((a = i.alternate),
  ...
  function Wd(s, i, a) {
    ...
    f = f.getChildContext();
    for (const h in f)
      if (!(h in i)) throw Error(n(108, Se(s) || 'Unknown', h));
    return { ...a, ...f };
  }
  function Io(s) {
    return (
  ...
  function Zd() {
    for (let s = At; s; ) s = Fn(s.nextSibling);
  }
  function Yr() {
    ((At = Nt = null), (Ie = !1));
  }
  ...
  const Pw = R.ReactCurrentBatchConfig;
  function fi(s, i, a) {
    if (
      ((s = a.ref),
  ...
  function Fo(s, i, a, f) {
    let h = s.updateQueue;
    Un = !1;
    let m = h.firstBaseUpdate;
    let x = h.lastBaseUpdate;
    let b = h.shared.pending;
    if (b !== null) {
  ...
  const zo = R.ReactCurrentDispatcher;
  const _c = R.ReactCurrentBatchConfig;
  let dr = 0;
  let Pe = null;
  let Ve = null;
  let Ge = null;
  ...
  function Ec(s, i) {
    if (i === null) return !1;
    for (let a = 0; a < i.length && a < s.length; a++)
  ...
  function Rt() {
    ...
    const i = Ge === null ? Pe.memoizedState : Ge.next;
    if (i !== null) ((Ge = i), (Ve = s));
    else {
  ...
  function Dw(s, i, a) {
    ...
    if (Th(s)) Ch(i, h);
      ...
      if (
        ...
        try {
          const x = i.lastRenderedState;
          const b = m(x, a);
          if (((h.hasEagerState = !0), (h.eagerState = b), Kt(b, x))) {
            const T = i.interleaved;
            (T === null
              ? ((h.next = h), gc(i))
  ...
  function Qt(s, i) {
    if (s && s.defaultProps) {
      ((i = { ...i }), (s = s.defaultProps));
      for (const a in s) i[a] === void 0 && (i[a] = s[a]);
      return i;
    }
    return i;
  }
...
${m.stack}`;
  ...
  const Hw = typeof WeakMap === 'function' ? WeakMap : Map;
  function Mh(s, i, a) {
  ...
  function jh(s, i, a) {
    ((a = wn(-1, a)), (a.tag = 3));
    const f = s.type.getDerivedStateFromError;
  ...
  function Bh(s, i, a) {
    let f = i.pendingProps;
    const h = f.children;
    const m = s !== null ? s.memoizedState : null;
    if (f.mode === 'hidden')
      if ((i.mode & 1) === 0)
        ((i.memoizedState = {
          baseLanes: 0,
  ...
  function Hh(s, i, a, f, h) {
    ...
    if ((ts(i, h), i.stateNode === null))
      ...
      const Y = a.getDerivedStateFromProps;
      ((V =
        typeof Y === 'function' ||
        typeof x.getSnapshotBeforeUpdate === 'function') ||
        (typeof x.UNSAFE_componentWillReceiveProps !== 'function' &&
  ...
  const $c = { dehydrated: null, treeContext: null, retryLane: 0 };
  function Rc(s) {
  ...
  function Vh(s, i, a) {
    ...
    if (m) {
      ...
      const T = { mode: 'hidden', children: f.children };
      return (
        (x & 1) === 0 && i.child !== h
          ? ((f = i.child),
            (f.childLanes = 0),
  ...
  function it(s) {
    const i = s.alternate !== null && s.alternate.child === s.child;
    let a = 0;
    let f = 0;
    if (i)
  ...
  let Qo = !1;
  let ot = !1;
  const Gw = typeof WeakSet === 'function' ? WeakSet : Set;
  let ee = null;
  function ss(s, i) {
    const a = s.ref;
    if (a !== null)
    ...
    if (((Za = mo), (s = Ad()), Va(s))) {
      if ('selectionStart' in s)
        ...
        e: {
          ...
          let f = a.getSelection && a.getSelection();
          if (f && f.rangeCount !== 0) {
            a = f.anchorNode;
            const h = f.anchorOffset;
            const m = f.focusNode;
            f = f.focusOffset;
            try {
          ...
          try {
            ...
            if ((i.flags & 1024) !== 0)
              switch (i.tag) {
                ...
                case 1:
                  if (te !== null) {
                    const ne = te.memoizedProps;
                    const Fe = te.memoizedState;
                    const L = i.stateNode;
                    const N = L.getSnapshotBeforeUpdate(
  ...
  function np(s) {
    const i = s.updateQueue;
    if (i !== null) {
      s.updateQueue = null;
      let a = s.stateNode;
      (a === null && (a = s.stateNode = new Gw()),
        i.forEach(function (f) {
          const h = s0.bind(null, s, f);
    ...
    switch (s.tag) {
      ...
      case 5:
        ...
        if (f & 4 && ((h = s.stateNode), h != null)) {
          ...
          var x = a !== null ? a.memoizedProps : m;
          var b = s.type;
          var T = s.updateQueue;
  ...
  function sp(s, i, a) {
    for (let f = (s.mode & 1) !== 0; ee !== null; ) {
      const h = ee;
      let m = h.child;
      if (h.tag === 22 && f) {
        let x = h.memoizedState !== null || Qo;
        if (!x) {
          let b = h.alternate;
          let T = (b !== null && b.memoizedState !== null) || ot;
          b = Qo;
          const P = ot;
          if (((Qo = x), (ot = T) && !P))
            for (ee = h; ee !== null; )
            ...
            switch (i.tag) {
              ...
              case 13:
                if (i.memoizedState === null) {
                  const P = i.alternate;
                  if (P !== null) {
                    const V = P.memoizedState;
                    if (V !== null) {
                      const W = V.dehydrated;
                      W !== null && Ys(W);
                    }
                  }
                }
  ...
  const Xw = Math.ceil;
  const Xo = R.ReactCurrentDispatcher;
  const Vc = R.ReactCurrentOwner;
  const Dt = R.ReactCurrentBatchConfig;
  var ve = 0;
  var Qe = null;
  let He = null;
  let Ze = 0;
  ...
  function St(s, i) {
    let a = s.callbackNode;
    Pv(s, i);
    const f = fo(s, s === Qe ? Ze : 0);
    if (f === 0)
      ...
      if (i === 6) Qn(s, f);
        ...
        switch (((s.finishedWork = h), (s.finishedLanes = f), i)) {
          ...
          case 3:
            if (
              (Qn(s, f),
              (f & 130023424) === f && ((i = Kc + 500 - De()), i > 10))
            ) {
              if (fo(s, 0) !== 0) break;
    ...
    do {
      ...
      try {
        if ((hc(), (zo.current = Vo), Ho)) {
          for (let f = Pe.memoizedState; f !== null; ) {
            const h = f.queue;
  ...
  function n0(s, i, a) {
    ...
    (f !== null && f.delete(i),
      ...
      Qe === s &&
        (Ze & a) === a &&
        (We === 4 || (We === 3 && (Ze & 130023424) === Ze && De() - Kc < 500)
          ? mr(s, 0)
          : (Wc |= a)),
      St(s, i));
  }
          ...
          if (
            ...
            if (Kt(m.value, x)) {
              ...
              for (m = i.child, m !== null && (m.return = i); m !== null; ) {
                let b = m.dependencies;
                if (b !== null) {
                  x = m.child;
                  for (let T = b.firstContext; T !== null; ) {
                    if (T.context === f) {
                      if (m.tag === 1) {
                        ((T = wn(-1, a & -a)), (T.tag = 2));
                        let P = m.updateQueue;
  ...
  ((nd = function (s) {
    ...
    (Uf = Yc),
    (qf = pr));
  const f0 = { usingClientEntryPoint: !1, Events: [ui, Kr, No, zf, Hf, Yc] };
  const ki = {
  ...
  if (typeof __REACT_DEVTOOLS_GLOBAL_HOOK__ < 'u') {
    const dl = __REACT_DEVTOOLS_GLOBAL_HOOK__;
  ...
  return (
    ...
    (xt.hydrateRoot = function (s, i, a) {
      ...
      const f = (a != null && a.hydratedSources) || null;
      let h = !1;
      let m = '';
      let x = xp;
      if (
...
const $m = Symbol('nextInContext');
const Rm = Symbol('prevByEndTime');
const Dm = Symbol('nextByStartTime');
const Pp = Symbol('events');
class Lk {
  constructor(e) {
    Ee(this, 'startTime');
    Ee(this, 'endTime');
...
function j0(t) {
  const e = new Map();
  const n = t.filter(c => c.origin === 'library');
  const r = t.filter(c => c.origin === 'testRunner');
...
const Dl = ({
  ...
  const [p, g] = Ts(l ? `${l}.${r}:size` : void 0, d);
  const [y, v] = Ts(l ? `${l}.${r}:size` : void 0, d);
          ...
          w.jsx('div', {
            ...
            onMouseMove: B => {
              if (!B.buttons) k(null);
                ...
                const z = n ? S.size + D : S.size - D;
                const F = B.target.parentElement.getBoundingClientRect();
                const M = Math.min(
                  Math.max(o, z),
...
function G0(t) {
  ...
  for (let n = 0; n < t.length; n++) {
    ...
    if (
      ...
      const o = r - 55296;
      const l = t.charCodeAt(n + 1) - 56320;
      ((r = 2 ** 16 + o * 2 ** 10 + l), n++);
    }
    e.push(r);
  }
...
function Ke(t) {
  ...
  t -= 2 ** 16;
  const e = Math.floor(t / 2 ** 10) + 55296;
  const n = (t % 2 ** 10) + 56320;
...
function Fm(t) {
  ...
  const C = function (O, X) {
    return !(O !== 92 || bl(X));
  };
  const A = function () {
    return C(o, c());
  };
  const B = function (O, X, ce) {
    return O === 45
      ? kl(X) || X === 45 || C(X, ce)
      : kl(O)
        ? !0
...
function n1(t, e) {
  ...
  try {
    ...
    const K = (M.stack || '').indexOf(M.message);
    throw (
      K !== -1 &&
        (M.stack =
...
const r1 = new Set(['left-of', 'right-of', 'above', 'below', 'near']);
const rg = new Set([
  'not',
  'is',
  'where',
...
function Ji(t) {
  ...
  for (const r of e.parts) {
    ...
    if (ju.has(r.name)) {
      ...
      const u = [...c.body.parsed.parts]
        .reverse()
        .find(p => p.name === 'internal:control' && p.body === 'enter-frame');
      const d = u ? c.body.parsed.parts.indexOf(u) : -1;
      (d !== -1 &&
...
function ea(t, e = "'") {
  ...
  const r = n.substring(1, n.length - 1).replace(/\\"/g, '"');
...
function Sr(t, e, n = !1, r = 20) {
  ...
  for (let u = 0; u < o.length; u++) {
    ...
    if (d.name === 'internal:testid') {
      const k = br(d.body, !0);
      const { value: _ } = k.attributes[0];
      l.push([t.generateLocator(p, 'test-id', _)]);
      continue;
    }
    if (d.name === 'internal:attr') {
      const k = br(d.body, !0);
      const { name: _, value: E, caseSensitive: C } = k.attributes[0];
      const A = E;
      const B = !!C;
      if (_ === 'placeholder') {
    ...
    const g = o[u + 1];
    const y = Tn({ parts: [d] });
    const v = t.generateLocator(p, 'default', y);
...
function ag({
  ...
  const R = p(t);
  const D = A && t.children.length ? t.children : [];
  const z = g == null ? void 0 : g(t);
  const H = (y == null ? void 0 : y(t)) || 'codicon-blank';
  return w.jsxs('div', {
    ref: _,
    role: 'treeitem',
    'aria-selected': t === n,
...
function v1(t, e, n, r, o = () => !0) {
  ...
  const d = (p, g) => {
    for (const y of p.children) {
      ...
      const S = r > g && l.size < 25 && v !== !1;
      const k = y.children.length ? (v ?? S) : void 0;
      const _ = {
        depth: g,
        expanded: k,
        parent: t === p ? null : p,
...
const k1 = ({
  ...
  const { rootItem: v, itemMap: S } = $.useMemo(() => $0(t), [t]);
  const { selectedItem: k } = $.useMemo(
    () => ({ selectedItem: e ? S.get(e.callId) : void 0 }),
    [S, e],
  );
  const _ = $.useCallback(D => {
    let z;
    let H;
    return !!(
...
const Zu = (t, e) => {
  ...
  const { errors: d, warnings: p } = D0(t);
  const g = !!((E = t.attachments) != null && E.length) && !!o;
  const y = t.params.selector
    ? u1(n || 'javascript', t.params.selector)
    : void 0;
  const v =
    t.class === 'Test' &&
    t.method === 'step' &&
...
const T1 = ({ action: t, startTimeOffset: e, sdkLanguage: n }) => {
  const r = $.useMemo(
    () =>
...
const N1 = na;
const A1 = ({ action: t, isLive: e }) => {
  const n = $.useMemo(() => {
    let c;
    if (!t || !t.log.length) return [];
    const r = t.log;
    const o = t.context.wallTime - t.context.startTime;
    const l = [];
...
function qi(t, e) {
  ...
  let l = {};
  let c = !1;
  let u = e == null ? void 0 : e.fg;
  let d = e == null ? void 0 : e.bg;
  for (; (o = n.exec(t)) !== null; ) {
    const [, , p, , g] = o;
    if (p) {
      const y = +p;
...
const j1 = { position: 'absolute', top: 0, right: 0, bottom: 0, left: 0 };
const fg = ({
  orientation: t,
  offsets: e,
  setOffsets: n,
            ...
            if (!v.buttons) d(null);
              ...
              const S =
                t === 'horizontal'
                  ...
                  : v.clientY - u.clientY;
              const k = u.offset + S;
              const _ = u.index > 0 ? e[u.index - 1] : 0;
              const E = t === 'horizontal' ? p.width : p.height;
              const C = Math.min(Math.max(_ + c, k), E - c) - e[u.index];
...
const P1 = ({ diff: t, noTargetBlank: e, hideDetails: n }) => {
  const [r, o] = $.useState(t.diff ? 'diff' : 'actual');
  const [l, c] = $.useState(!1);
  const [u, d] = $.useState(null);
  const [p, g] = $.useState('Expected');
  const [y, v] = $.useState(null);
  const [S, k] = $.useState(null);
  const [_, E] = Ar();
  $.useEffect(() => {
    (async () => {
  ...
  const C = u && y && S;
  const A = C ? Math.max(u.naturalWidth, y.naturalWidth, 200) : 500;
  const B = C ? Math.max(u.naturalHeight, y.naturalHeight, 200) : 500;
  const R = Math.min(1, (_.width - 30) / A);
  const D = Math.min(1, (_.width - 50) / A / 2);
  const z = A * R;
  const H = B * R;
  const F = {
    flex: 'none',
...
const O1 = ({
  ...
  const [d, p] = $.useState(n / 2);
  const g =
    t.naturalWidth === e.naturalWidth && t.naturalHeight === e.naturalHeight;
  return w.jsxs('div', {
    style: {
      flex: 'none',
      display: 'flex',
...
const R1 = function (t, e) {
  return new URL(t, e).href;
};
const Qp = {};
const D1 = function (e, n, r) {
...
const K1 = ({ attachment: t, reveal: e }) => {
  ...
  const [c, u] = $.useState(null);
  const [d, p] = _0();
  const g = $.useRef(null);
  const y = q1(t.contentType);
  const v = !!t.sha1 || !!t.path;
  ($.useEffect(() => {
    let _;
    if (e)
      return (
...
const G1 = ({ model: t, revealedAttachment: e }) => {
  const {
    diffMap: n,
    screenshots: r,
    attachments: o,
...
const eS = na;
const tS = ({ stack: t, setSelectedFrame: e, selectedFrame: n }) => {
  const r = t || [];
  return w.jsx(eS, {
    name: 'stack-trace',
  ...
  return $l(
    async () => {
      ...
      const p =
        ...
        0;
      const g = r && u.startsWith(r) ? u.substring(r.length + 1) : u;
      const y = d.errors.map(E => ({
        type: 'error',
        line: E.line,
        message: E.message,
      }));
...
const rS = ({
  ...
  const E = ((t == null ? void 0 : t.length) ?? 0) > 1;
  const C = sS(S);
  return w.jsx(Dl, {
    sidebarSize: 200,
...
async function pg(t) {
  ...
  const n = await crypto.subtle.digest('SHA-1', e);
  const r = [];
...
`,
    ...
    .slice();
  const c = Math.max(0, n.line - r - 1);
  const u = Math.min(l.length, n.line + o);
  const d = l.slice(c, u);
  const p = String(u).length;
  const g = d.map(
    (y, v) =>
...
function fS(t, e) {
  const { entries: n } = $.useMemo(() => {
    ...
    const c = [...t.events, ...t.stdio].sort((u, d) => {
      ...
      const g = 'time' in d ? d.time : d.timestamp;
      return p - g;
    });
    for (const u of c) {
      if (u.type === 'console') {
        const d = u.args && u.args.length ? hS(u.args) : mg(u.text);
        const p = u.location.url;
        const y = `${p ? p.substring(p.lastIndexOf('/') + 1) : '<anonymous>'}:${u.location.lineNumber}`;
        l({
    ...
    ? w.jsx('div', {
        ...
        children: w.jsx(uS, {
          ...
          render: o => {
            ...
            const c = w.jsx('span', { className: 'console-time', children: l });
            const u = o.isError
              ? 'status-error'
              : o.isWarning
                ? 'status-warning'
...
async function yS(t, e = 0) {
  ...
  const r = new Set(['cookie', 'authorization']);
  const o = JSON.stringify(t.request.url);
  const l = t.request.headers;
  const c = l.reduce((k, _) => {
    const E = _.name;
    return (
  ...
  const d =
    t.request.cookies.length || l.some(({ name: k }) => r.has(k.toLowerCase()))
      ? 'include'
      : 'omit';
  const p = l.find(({ name: k }) => k.toLowerCase() === 'referer');
  const g = p ? p.value : void 0;
  const y = await yg(t);
  const v = {
...
${u}]`;
    ...
    const o = ' '.repeat(r * n);
    const l = ' '.repeat((r + 1) * n);
    return `{
${Object.entries(e).map(([u, d]) => {
  const p = this.prettyPrintObject(d, n, r + 1);
  const g = /^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(u) ? u : this.stringLiteral(u);
  return `${l}${g}: ${p}`;
}).join(`,
`)}
${o}}`;
...
${Object.entries(e).map(([u, d]) => {
  const p = this.prettyPrintObject(d, n, r + 1);
  const g = r === 0 ? u : `[${this.stringLiteral(u)}]`;
  return `${l}${g} = ${p}`;
}).join(`,
`)}
${o}}`;
...
const kS = ({ resource: t, sdkLanguage: e, requestBody: n }) => {
  ...
  const o = async () =>
    _S(e).generatePlaywrightRequestCall(t.request, n == null ? void 0 : n.text);
  return w.jsxs('div', {
...
const bS = ({ resource: t, startTimeOffset: e, requestBody: n }) =>
  w.jsxs('div', {
    ...
    $.useEffect(() => {
      (async () => {
        if (t.response.content._sha1) {
          ...
          const l = t.response.content.mimeType.includes('font');
          const c = await fetch(`sha1/${t.response.content._sha1}`);
          if (o) {
            const u = await c.blob();
            const d = new FileReader();
            const p = new Promise(g => (d.onload = g));
...
const jS = { searchValue: '', resourceType: 'All' };
const PS = ({ filterState: t, onFilterStateChange: e }) =>
  w.jsxs('div', {
...
const RS = ({
  ...
  const [c, u] = $.useState(void 0);
  const [d, p] = $.useState(jS);
  const { renderedEntries: g } = $.useMemo(() => {
  ...
  const [y, v] = $.useState(() => new Map(vg().map(_ => [_, FS(_)])));
  const S = $.useCallback(_ => {
    (p(_), u(void 0));
  }, []);
...
function nf(t, e, n = {}) {
  ...
  const o = { keepSourceTokens: !0, lineCounter: r, ...n };
  const l = t.parseDocument(e, o);
  const c = [];
  const u = S => [r.linePos(S[0]), r.linePos(S[1])];
  const d = S => {
    c.push({
      message: S.message,
  ...
  const y = { kind: 'role', role: 'fragment' };
  return (
    l.errors.forEach(d),
    c.length
      ? { errors: c, fragment: y }
...
`;
  ...
  _parse() {
    ...
    const r = { kind: 'role', role: e, name: n };
    return (
  ...
  if (!['presentation', 'none'].includes(o)) {
    ...
    if (!r && (l === 'TEXTAREA' || l === 'SELECT' || l === 'INPUT')) {
      ...
      const S =
        ...
        l === 'TEXTAREA';
      const k = t.getAttribute('placeholder') || '';
...
function $g(t) {
  ...
  if (cf.includes(nt(t) || '')) {
    ...
    const r = n === null ? Number.NaN : Number(n);
...
function Gl(t, e) {
  const n = [];
  const r = (e == null ? void 0 : e.mode) === 'regex' ? Tx : () => !0;
  const o = (e == null ? void 0 : e.mode) === 'regex' ? bx : u => u;
  const l = (u, d, p) => {
    if (typeof u === 'string') {
      if (d && !r(d, u)) return;
      const S = yu(o(u));
    ...
    if (
      ...
      const S = u.ref;
      const k = Cx(u) ? ' [cursor=pointer]' : '';
      S && (g += ` [ref=${S}]${k}`);
    }
    const y = `${p}- ${gx(g)}`;
    const v = !!Object.keys(u.props).length;
...
class vu {
  ...
  runHighlightOnRaf(e) {
    ...
    const r = Tr(this._language, Tn(e));
    const o = n.length > 1 ? '#f6b26b7f' : '#6fa8dc7f';
  ...
  tooltipPosition(e, n) {
    ...
    const l = this._glassPaneElement.offsetWidth;
    const c = this._glassPaneElement.offsetHeight;
    let u = e.left;
    u + r > l - 5 && (u = l - r - 5);
    let d = e.bottom + 5;
    return (
...
function qg(t, e, n, r) {
  ...
  const l = { 'left-of': Ax, 'right-of': Nx, above: Ix, below: Lx, near: Mx }[
    t
  ];
  let c;
  for (const u of n) {
...
class zx {
  ...
  _cached(e, n, r, o) {
    e.has(n) || e.set(n, []);
    const l = e.get(n);
    const c = l.find(d => r.every((p, g) => d.rest[g] === p));
...
const Kx = {
  matches(t, e, n, r) {
    ...
    const l = c => c.normalized.toLowerCase().includes(o);
    ...
    matches(e, n, r, o) {
      const l =
        n.length && typeof n[n.length - 1] === 'number'
          ? n[n.length - 1]
          : void 0;
      const c = l === void 0 ? n : n.slice(0, n.length - 1);
...
function us(t, e, n, r) {
  ...
  const o = (c, u) => {
    ...
    const v = S => {
      const k = u && !S.length;
      const _ = [...S, ...g].filter(C => (y ? Zn(C) < Zn(y) : !0));
      let E = _[0];
      if (E)
...
function ml(t, e, n) {
  ...
  function l(u) {
    ...
    u && d.unshift(u);
    const p = d.join(' > ');
    const g = t.parseSelector(p);
    return t.querySelector(g, r, !1) === e ? p : void 0;
  }
  function c(u) {
    const d = { engine: 'css', selector: u, score: c_ };
    const p = t.parseSelector(u);
    const g = t.querySelectorAll(p, r);
...
function pm(t, e, n, r, o) {
  ...
  for (const { tokens: u } of l) {
    ...
    const p = t.querySelectorAll(d, e);
...
const S_ = () => ({
  queryAll(t, e) {
    ...
    const { name: r, attributes: o } = br(e, !1);
    const u = dy(n)
...
const N_ = class $i {
  constructor(e, n, r) {
    ...
    ((l.locator = (c, u) => new $i(e, o ? `${o} >> ${c}` : c, u)),
      ...
      (l.and = c => new $i(e, `${o} >> internal:and=${JSON.stringify(c[Ii])}`)),
      (l.or = c => new $i(e, `${o} >> internal:or=${JSON.stringify(c[Ii])}`)));
  }
};
const A_ = N_;
class I_ {
...
class my {
  ...
  _queryLayoutSelector(e, n, r) {
    ...
    const l = n.body;
    const c = [];
    const u = this.querySelectorAll(l.parsed, r);
    for (const d of e) {
  ...
  querySelectorAll(e, n) {
    if (e.capture !== void 0) {
      ...
      if (e.capture < e.parts.length - 1) {
        ...
        const l = { name: 'internal:has', body: { parsed: o }, source: Tn(o) };
        r.parts.push(l);
      }
  ...
  _createInternalHasTextEngine() {
    return {
      queryAll: (e, n) => {
        ...
        const r = e;
        const o = Tt(this._evaluator._cacheText, r);
        const { matcher: l } = vl(n, !0);
        return l(o) ? [r] : [];
      },
    };
  }
  ...
  async _checkElementIsStable(e) {
    ...
    const c = () => {
      ...
      const k = { x: S.top, y: S.left, width: S.width, height: S.height };
      if (r) {
        if (
          !(
            k.x === r.x &&
      ...
      const g = y => {
        ...
        return (
          y.valueOrLabel !== void 0 &&
            (v =
              v && (y.valueOrLabel === p.value || y.valueOrLabel === p.label)),
  ...
  _activelyFocused(e) {
    ...
    const r = n === e && !!e.ownerDocument && e.ownerDocument.hasFocus();
  ...
  expectHitTarget(e, n) {
    ...
    for (let g = r.length - 1; g >= 0; g--) {
      const y = r[g];
      const v = y.elementsFromPoint(e.x, e.y);
      const S = y.elementFromPoint(e.x, e.y);
      if (S && v[0] && lt(S) === v[0]) {
...
`)
  ...
  markTargetElements(e, n) {
    ...
    const o = new CustomEvent('__playwright_unmark_target__', {
      bubbles: !0,
      cancelable: !0,
      detail: n,
      composed: !0,
  ...
  async expect(e, n, r) {
    ...
    if (!e) {
      ...
      if (
        ...
        const u = new fs(n.expectedText[0]);
        const d = this.document.location.href;
        return { received: d, matches: u.matches(d) };
      }
  ...
  expectArray(e, n) {
    ...
    const l = r !== 'to.contain.text.array';
...
function vl(t, e) {
  if (t[0] === '/' && t.lastIndexOf('/') > 0) {
    ...
    const l = new RegExp(t.substring(1, o), t.substring(o + 1));
...
class H_ {
  ...
  _actionInProgress(e) {
    ...
    const r = e instanceof MouseEvent || e instanceof PointerEvent;
...
class U_ {
  ...
  onClick(e) {
    ...
    if (
      ...
      return;
    const r = Hi(n);
    const { ariaSnapshot: o, selector: l } = this._ariaSnapshot(n);
    if (r && e.detail === 1) {
      this._recorder.recordAction({
  ...
  _ariaSnapshot(e) {
    ...
    const o = e ? r.get(e) : void 0;
    const l = o ? `aria-ref=${o}` : void 0;
...
class Eu {
  ...
  _showTextDialog(e) {
    ...
    const r = () => {
      ...
      const d = (y = this._hoverHighlight) == null ? void 0 : y.elements[0];
      if (!d) return;
      e.text = u;
    ...
    const c = this._recorder.highlight.tooltipPosition(
...
class W_ {
  ...
  show(e) {
    ...
    const l = this._recorder.document.createElement('label');
...
function G_(t, e, n, r) {
  try {
    ...
    const l = t.querySelectorAll(o, r);
    const c = l.length > 1 ? Ht.multiple : Ht.single;
    const u = t.utils.asLocator(e, n);
    return l.map((d, p) => {
...
function vy(t, e, n) {
  for (;;) {
    ...
    const y = vm(t.substring(l, u), g);
    const v = ym(y);
    const S = e.slice(g, g + v);
    const k = JSON.stringify(vy(y, S, n));
...
function X_(t, e, n) {
  ...
  const l = og(t, r, void 0, void 0, o);
  const c = wm(t, e);
...
const kf = Symbol.for('yaml.alias');
const zu = Symbol.for('yaml.document');
const er = Symbol.for('yaml.map');
const wy = Symbol.for('yaml.pair');
const dn = Symbol.for('yaml.scalar');
const Ps = Symbol.for('yaml.seq');
const Vt = Symbol.for('yaml.node.type');
const Lr = t => !!t && typeof t === 'object' && t[Vt] === kf;
const Mr = t => !!t && typeof t === 'object' && t[Vt] === zu;
const Os = t => !!t && typeof t === 'object' && t[Vt] === er;
const Me = t => !!t && typeof t === 'object' && t[Vt] === wy;
const ke = t => !!t && typeof t === 'object' && t[Vt] === dn;
const $s = t => !!t && typeof t === 'object' && t[Vt] === Ps;
function Oe(t) {
  if (t && typeof t === 'object')
    switch (t[Vt]) {
      case er:
...
const bt = Symbol('break visit');
const Sy = Symbol('skip children');
const fn = Symbol('remove node');
function tr(t, e) {
  const n = xy(e);
  Mr(t)
...
class dt {
  ...
  toString(e) {
    ...
    const r = Object.entries(this.tags);
    let o;
...
function Ui(t, e) {
  ...
  const o = e.options.doubleQuotedMinMultiLineLength;
  const l = e.indent || (ua(t) ? '  ' : '');
  let c = '';
  let u = 0;
...
${d}`,
  ...
  if (c) {
    ...
    const { compat: S, tags: k } = e.doc.schema;
...
function Xi(t, e, n, r) {
  ...
  const c = typeof t.value === 'string' ? t : { ...t, value: String(t.value) };
  let { type: u } = t;
  u !== ue.QUOTE_DOUBLE &&
...
`);
    const M = F !== -1;
    const G = n.inFlow ?? e.flow ?? e.items.length === 0;
    if (M || !G) {
      let K = !1;
      if (M && (H === '&' || H === '!')) {
        let O = D.indexOf(' ');
...
class Lt extends Ny {
  ...
  add(e, n) {
    ...
    const l = (c = this.schema) == null ? void 0 : c.sortMapEntries;
    if (o) {
...
const ha = t => typeof t === 'bigint' || Number.isInteger(t);
const Nf = (t, e, n, { intAsBigInt: r }) =>
...
const mE = [Rs, Ds, fa, da, Cf, Fy, By, zy, Oy, $y, Ry];
function xm(t) {
...
const Yi = t => typeof t === 'bigint' || Number.isInteger(t);
...
function Pf(t, e) {
  const n = t[0];
  const r = n === '-' || n === '+' ? t.substring(1) : t;
  const o = c => (e ? BigInt(c) : Number(c));
  const l = r
    .replace(/_/g, '')
    .split(':')
...
const ma = {
  ...
  resolve(t) {
    ...
    const d = e[7] ? Number(`${e[7]}00`.substr(1, 3)) : 0;
...
function NE(t, e) {
  ...
  r && n.push('---');
  const o = Iy(t, e);
  const { commentString: l } = o.options;
  if (t.commentBefore) {
...
class Fs {
  ...
  createNode(e, n, r) {
    ...
    const { onAnchor: y, setAnchors: v, sourceObjects: S } = nE(this, c || 'a');
    const k = {
  ...
  createPair(e, n, r = {}) {
    ...
    const l = this.createNode(n, null, r);
    return new at(o, l);
  }
...
function As(
  ...
  const H = z ? z.offset + z.source.length : o;
  return (
    k &&
      r &&
      r.type !== 'space' &&
...
${E.comment}`)
    ...
    n.atKey = !0;
    const A = E.end;
    const B = S ? t(n, S, E, o) : e(n, A, v, null, E, o);
...
const Cu = t => t && (t.type === 'block-map' || t.type === 'block-seq');
function LE({ composeNode: t, composeEmptyNode: e }, n, r, o, l) {
  const c = r.start.source === '{';
  const u = c ? 'flow map' : 'flow sequence';
  const d = (l == null ? void 0 : l.nodeClass) ?? (c ? Lt : nr);
  const p = new d(n.schema);
  p.flow = !0;
  const g = n.atRoot;
  ...
  for (let E = 0; E < r.items.length; ++E) {
    const C = r.items[E];
    const { start: A, key: B, sep: R, value: D } = C;
    const z = As(A, {
      flow: u,
      indicator: 'explicit-key-ind',
...
${H}`)
    ...
    if (!c && !R && !z.found) {
      ...
      n.atKey = !0;
      const H = z.end;
      const F = B ? t(n, B, z, o) : e(n, H, A, null, z, o);
...
function ME(t, e, n, r, o) {
  ...
  if (n.type === 'block-seq') {
    ...
    const _ = S && l ? (S.offset > l.offset ? S : l) : (S ?? l);
    _ &&
      (!k || k.offset < _.offset) &&
...
function PE(t) {
  ...
  const n = e[0];
  const r = n.match(/^( *)/);
  const l = [r != null && r[1] ? [r[1], n.slice(r[1].length)] : ['', n]];
...
function BE(t, e, n, r) {
  ...
  const c = o.length === n && /^[0-9a-fA-F]+$/.test(o) ? parseInt(o, 16) : NaN;
  if (isNaN(c)) {
...
function HE({ atKey: t, directives: e, schema: n }, r, o, l) {
  ...
  if (n.compat) {
    ...
    if (c.tag !== u.tag) {
      ...
      const p = e.tagString(u.tag);
      const g = `Value may be parsed as either ${d} or ${p}`;
...
function nv(t, e, n, r) {
  const o = t.atKey;
  const { spaceBefore: l, comment: c, anchor: u, tag: d } = n;
  let p;
  let g = !0;
  switch (e.type) {
    case 'alias':
    ...
    switch (e.type) {
      ...
      case 'error': {
        const n = e.source
          ...
          : e.message;
        const r = new kr(Li(e), 'UNEXPECTED_TOKEN', n);
...
`,
  ...
  switch (u[0]) {
    ...
    case '>': {
      const p = u.indexOf(`
`);
      const g = u.substring(0, p);
      const y = `${u.substring(p + 1)}
`;
      const v = [
        { type: 'block-scalar-header', offset: l, indent: r, source: g },
      ];
      return (
        rv(v, d) ||
          v.push({
...
function QE(t, e, n = {}) {
  ...
  let u = 'indent' in t ? t.indent : null;
...
`;
  if (t.type === 'block-scalar') {
    ...
    const { offset: l } = t;
    const c = 'indent' in t ? t.indent : -1;
    const u = [
      { type: 'block-scalar-header', offset: l, indent: c, source: r },
    ];
...
const YE = Symbol('skip children');
const sv = Symbol('remove item');
function Nr(t, e) {
  ('type' in t &&
    t.type === 'document' &&
...
`,
  ...
  *blockMap(e) {
    ...
    switch (this.type) {
      case 'newline':
        if (((this.onKeyLine = !1), n.value)) {
          ...
          const l = Array.isArray(o) ? o[o.length - 1] : void 0;
      ...
      case 'comment':
        if (n.value) e.items.push({ start: [this.sourceToken] });
          ...
          if (this.atIndentedComment(n.start, e.indent)) {
            ...
            const l =
              (r = o == null ? void 0 : o.value) == null ? void 0 : r.end;
            if (Array.isArray(l)) {
    ...
    if (this.indent >= e.indent) {
      ...
      const l = o && (n.sep || n.explicitKey) && this.type !== 'seq-item-ind';
      let c = [];
      if (l && n.sep && !n.value) {
        const u = [];
...
function ik(t, e = {}) {
  ...
  const o = new Df(n == null ? void 0 : n.addNewLine);
  const l = new Rf(e);
  const c = Array.from(l.compose(o.parse(t)));
  if (r && n)
    for (const u of c)
...
const ak = ({
  ...
  const [p, g] = Ts('shouldPopulateCanvasFromScreenshot', !1);
  const y = $.useMemo(() => dk(t), [t]);
  const v = $.useMemo(() => {
    const S = y[u];
...
const ck = ({
  ...
  const u = $.useRef(null);
  const d = $.useRef(null);
  const [p, g] = $.useState({ viewport: mv, url: '' });
  const y = $.useRef({ iteration: 0, visibleIframe: 0 });
  return (
    $.useEffect(() => {
      (async () => {
        const v = y.current.iteration + 1;
        const S = 1 - y.current.visibleIframe;
...
const uk = ({ snapshotInfo: t, children: e }) => {
  const [n, r] = Ar();
  const l = { width: t.viewport.width, height: t.viewport.height + 40 };
  const c = Math.min(n.width / l.width, n.height / l.height, 1);
  const u = { x: (n.width - l.width) / 2, y: (n.height - l.height) / 2 };
  return w.jsx('div', {
    ref: r,
...
const Im = ({
  ...
  $.useEffect(() => {
    ...
    const d = o.lastEdited === 'locator' ? o.locator : void 0;
    const p = !!u || !!d || e;
    const g = [];
    const y =
      new URLSearchParams(window.location.search).get('isUnderTest') === 'true';
    try {
      hv(g, p, n, r, y, '', t == null ? void 0 : t.contentWindow);
    } catch {}
    const v = u ? nf(dv, u) : void 0;
    const S = d ? J_(n, d, r) : void 0;
    for (const { recorder: k, frameSelector: _ } of g) {
      const E =
        S != null && S.startsWith(_) ? S.substring(_.length).trim() : void 0;
      const C =
        (v == null ? void 0 : v.errors.length) === 0 ? v.fragment : void 0;
      k.setUIState(
        {
...
const Lm = new URLSearchParams(window.location.search).get('server');
function hk(t, e) {
...
const mk = 'data:text/html,<body style="background: #ddd"></body>';
const gv = { width: 200, height: 45 };
const ms = 2.5;
const gk = gv.height + ms * 2;
const yk = ({ model: t, boundaries: e, previewPoint: n }) => {
  let g;
  let y;
  const [r, o] = Ar();
  const l = $.useRef(null);
...
const vk = ({ boundaries: t, page: e, width: n }) => {
  const r = { width: 0, height: 0 };
  const o = e.screencastFrames;
  for (const _ of o)
    ((r.width = Math.max(r.width, _.width)),
      (r.height = Math.max(r.height, _.height)));
  const l = vv(r, gv);
  const c = o[0].timestamp;
  const u = o[o.length - 1].timestamp;
  const d = t.maximum - t.minimum;
  const p = ((c - t.minimum) / d) * n;
  const g = ((t.maximum - u) / d) * n;
  const v = ((((u - c) / d) * n) / (l.width + 2 * ms)) | 0;
  const S = (u - c) / v;
  const k = [];
...
const wk = ({
  ...
  const {
    ...
    if (v && v.startX !== v.endX) {
      ...
      const Ae = un(g.width, e, v.endX);
      G = { minimum: Math.min(ce, Ae), maximum: Math.max(ce, Ae) };
    }
    const K = Bt(g.width, e, G.minimum);
    const X = Bt(g.width, e, e.maximum) - Bt(g.width, e, G.maximum);
  ...
  const B = $.useMemo(() => {
    ...
    for (const K of (t == null ? void 0 : t.resources) || []) {
      ...
      const X = K._monotonicTime + K.time;
      G.push({
        resource: K,
        leftTime: O,
        rightTime: X,
  ...
  const R = $.useCallback(
    G => {
      ...
      const O = un(g.width, e, K);
      const X = u ? Bt(g.width, e, u.minimum) : 0;
      const ce = u ? Bt(g.width, e, u.maximum) : 0;
      u && Math.abs(K - X) < 10
...
const Mk = ({
  ...
  const [k, _] = $.useState(void 0);
  const [E, C] = $.useState(void 0);
  const [A, B] = $.useState();
  const [R, D] = $.useState();
  const [z, H] = $.useState();
  const [F, M] = $.useState('actions');
  const [G, K] = Ts('propertiesTab', e ? 'source' : 'call');
  const [O, X] = $.useState(!1);
  const [ce, Ae] = $.useState({ lastEdited: 'none' });
  const [be, ge] = $.useState();
  const [J, se] = Ts('propertiesSidebarLocation', 'bottom');
  const Z = $.useCallback(le => {
  ...
  const U = $.useCallback(le => {
    B(le == null ? void 0 : le.callId);
  }, []);
  const de = $.useMemo(
    () => (t == null ? void 0 : t.sources) || new Map(),
    [t],
  );
  $.useEffect(() => {
    (ge(void 0), _(void 0));
  ...
  const sr = $S(t, be);
  const Or = oS(t);
  const hn = (t == null ? void 0 : t.sdkLanguage) || 'javascript';
  const eo = {
    id: 'inspector',
    title: 'Locator',
    render: () =>
  ...
  const In = [eo, Bs, to, zs, Hs, no, nn, ro];
  if (u !== void 0) {
    const le = {
      id: 'annotations',
      title: 'Annotations',
...
class Pk {
  constructor(e) {
    ((this._onCloseEmitter = new Mi()),
      ...
      this._transport.onmessage(r => {
        ...
        const { id: l, result: c, error: u, method: d, params: p } = o;
        if (l) {
  ...
  async _sendMessage(e, n) {
    ...
    const l = { id: o, method: e, params: n };
    return (
