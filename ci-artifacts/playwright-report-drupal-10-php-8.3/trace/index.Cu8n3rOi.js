...
const $ = () => {
  ...
  const [d, u] = s.useState([]);
  const [h, T] = s.useState(D);
  const [y, w] = s.useState({ done: 0, total: 0 });
  const [b, x] = s.useState(!1);
  const [S, g] = s.useState(null);
  const [p, j] = s.useState(null);
  const L = s.useCallback(t => {
    const c = [];
    const l = [];
  ...
  (s.useEffect(() => {
    ...
    if (t.has('isServer')) {
      ...
      const i = new URL(`../${l}`, window.location.toString());
...
Make sure to serve the Trace Viewer (${window.location}) via HTTPS or localhost.`);
  ...
  const o = n.get('trace');
  const a = (o == null ? void 0 : o.endsWith('.json'))
    ? e.jsx(G, { traceJson: o })
    : e.jsx($, {});
