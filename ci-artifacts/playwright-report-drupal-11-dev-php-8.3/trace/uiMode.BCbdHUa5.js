...
class ot {
  ...
  _onStepBegin(t, e, s) {
    ...
    const c = i.results.find(h => h._id === e);
    const n = s.parentStepId ? c._stepMap.get(s.parentStepId) : void 0;
    const g = this._absoluteLocation(s.location);
    const l = new te(s, n, g, c);
  ...
  _onStepEnd(t, e, s) {
    ...
    let l;
    const i = this._tests.get(t);
    const c = i.results.find(_ => _._id === e);
    const n = c._stepMap.get(s.id);
    ((n._endPayload = s),
  ...
  _onStdIO(t, e, s, i, c) {
    ...
    const g = e ? this._tests.get(e) : void 0;
    const l = g && s ? g.results.find(u => u._id === s) : void 0;
    t === 'stdout'
...
class nt {
  ...
  _fileItem(t, e) {
    ...
    const i = this._treeItemById.get(s);
    if (i) return i;
...
const ue = ({ tag: r, style: t, onClick: e }) =>
  o.jsx('span', {
...
const fe = ({
  ...
  const [L, O] = d.useState(h);
  const [$, N] = d.useState(v);
  d.useEffect(() => {
    if (L !== h) {
      ...
      if (!_)
        if (n) e == null || e.watchNoReply({ fileNames: s.fileNames() });
          ...
          for (const E of g.value) {
            ...
            const M = x == null ? void 0 : x.location.file;
            M && f.add(M);
          }
...
const me = ({
  ...
  const [g, l] = d.useState(0);
  const _ = d.useRef(null);
  const { outputDir: a } = d.useMemo(
...
const we = new URL(A.get('server') ?? '../', window.location.href);
const lt = new URL(A.get('ws'), we);
...
const ve = ({}) => {
  ...
  const [e, s] = d.useState(!1);
  const [i, c] = d.useState(!1);
  const [n, g] = d.useState(
    new Map([
      ['passed', !1],
      ['failed', !1],
      ['skipped', !1],
  ...
  const [l, _] = d.useState(new Map());
  const [a, h] = d.useState();
  const [v, u] = d.useState();
  const [b, S] = d.useState({});
  const [B, C] = d.useState(new Set());
  const [T, L] = d.useState(!1);
  const [O, $] = d.useState();
  const N = O && !O.completed;
  const [R, J] = $t('watch-all', !1);
  const [H, f] = d.useState({ value: new Set() });
  const E = d.useRef(Promise.resolve());
  const x = d.useRef(new Set());
  const [M, q] = d.useState(0);
  const [Z, xt] = d.useState(0);
  const [Tt, kt] = d.useState(!1);
  const [ct, ut] = d.useState(!0);
  const [w, jt] = d.useState();
  const [G, Et] = d.useState();
  const [tt, yt] = d.useState(!1);
  const [be, Se] = d.useState(!1);
  const [It, dt] = d.useState(!1);
  const Rt = d.useCallback(() => dt(!0), [dt]);
  const Bt = !1;
  const [ht, xe] = d.useState(!1);
  const [ft, Te] = d.useState(!1);
  const [pt, ke] = d.useState(!1);
  const Ct = d.useRef(null);
...
`,
  ...
  (d.useEffect(() => {
    ...
    const p = w.onTestFilesChanged(async m => {
      if (
        ...
        return;
      const k = G.asModel();
      const y = new nt('', k.rootSuite, k.loadErrors, l, I.pathSeparator);
      const P = [];
      const U = new Set(m.testFiles);
      if (R) {
        const D = j => {
        ...
        for (const D of H.value) {
          ...
          const W = j == null ? void 0 : j.location.file;
          W && U.has(W) && P.push(...y.collectTestIds(j));
        }
  ...
  const Nt = d.useCallback(p => {
    let m;
    (p.preventDefault(),
      p.stopPropagation(),
      (m = it.current) == null || m.showModal());
  }, []);
  const rt = d.useCallback(p => {
    let m;
    (p.preventDefault(),
      p.stopPropagation(),
      (m = it.current) == null || m.close());
  }, []);
  const Lt = d.useCallback(
    p => {
      (rt(p),
        s(!0),
        w == null ||
